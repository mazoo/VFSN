...
class replace_class {
	...
	function do_explode($text)
	{
		...
		#$split_array=array(1=>" ");
		$text_array = explode(" ", $text);
		#foreach ($text_array as $text2){

		#	foreach ($split_array as $split){
		#   $text_array2 = explode($split, $text2);
		#   $textmerge=array_merge($textmerge,$text_array2);
		#	}
		#	}
		#$textmerge=array_merge($textmerge,$text_array);
	...
	function abk_to_print($block)
	{
		...
		if (!empty ($lqy)) {
			...
			$new_block = str_ireplace("___", " ", $block);
