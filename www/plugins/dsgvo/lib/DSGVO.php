...
class DSGVO
{
	...
	private function removeFormData() {
		...
		$GLOBALS["db"]->query(
			"DELETE lead, content ".
			"FROM {$GLOBALS["cms"]->tbname["papoo_form_manager_leads"]} lead ".
			"LEFT JOIN {$GLOBALS["cms"]->tbname["papoo_form_manager_lead_content"]} content ON content.form_manager_content_lead_id_id = lead.form_manager_lead_id ".
			"WHERE DATE(FROM_UNIXTIME(lead.form_manager_form_datum)) <= '$expire'"
		);
	}
