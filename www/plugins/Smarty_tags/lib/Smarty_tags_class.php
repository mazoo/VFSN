...
class Smarty_tags_class
{
	...
	{
		$this->content->template['ok'] = $this->checked->ok; // msg ctrl
		...
		elseif (isset($this->checked->lock)) $this->content->template['error9'] = $fehler = 1; //wrong activate content
		if (!$fehler) {
			// delete a user tag
	...
	function Smarty_tags_add_edit_user_tags()
	{
		...
		if (isset($this->checked->submit)) { // data from edit or add
			...
			if (!$fehler && $template2 == "Smarty_tags_add_user_tags_back.html") { // add the user tag to database now
			...
			elseif (!$fehler && $template2 == "Smarty_tags_edit_user_tags_back.html") { // update the user tag now
				$sql = sprintf("UPDATE %s
								SET user_plugin_name = '%s',
									php_code = '%s',
									mod_date = '%s'
		...
		if (!$fehler AND $template2 == "Smarty_tags_edit_user_tags_back.html") { // edit user tag) // read a user tag from database for edit
		...
		elseif ($fehler) { // restore user inputs if errors in inputs
	...
	function Smarty_tags_add_edit_papoo_vars()
	{
		...
		if (isset($this->checked->submit)) { // data from edit or add
			...
			if (!$fehler && $template2 == "Smarty_tags_add_papoo_vars_back.html") { // add the Papoo var to database now
				$sql = sprintf("INSERT INTO %s
			...
			elseif (!$fehler && $template2 == "Smarty_tags_edit_papoo_vars_back.html") { // update the Papoo var now
				$sql = sprintf("UPDATE %s
								SET papoo_var_name = '%s',
									papoo_var_descript = '%s',
									mod_date = '%s'
		...
		if (!$fehler AND $template2 == "Smarty_tags_edit_papoo_vars_back.html") {
